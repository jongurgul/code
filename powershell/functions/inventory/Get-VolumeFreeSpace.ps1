...
#>
    ...
    ForEach ($Computer in $Computers) {
        ...
        $MountPoint = Get-WmiObject -Class "Win32_MountPoint" -ComputerName $Computer | Select-Object @{Name = "DeviceID"; Expression = {$_.Volume.ToString().Substring($_.Volume.ToString().IndexOf("`"")).Replace("`"", "").Replace("\\", "\")}}, @{Name = "MountPoint"; Expression = {$_.Directory.ToString().Substring($_.Volume.ToString().IndexOf("`"")).Replace("`"", "").Replace("\\", "\")}};
        [String[]]$Mounts = $MountPoint| ForEach-Object {$_.DeviceID}
        $Volume | ForEach-Object {
            ...
            $DeviceID = $_.DeviceID;
            $MountPoint| Where-Object {$DeviceID -contains $_.DeviceID}| ForEach-Object {
                $Local = $Entry | Select-Object *
                $Local.MountPoint = $_.MountPoint;
                [Void]$Results.Add($Local);
            };
            $_|Where-Object {$Mounts -notcontains $_.DeviceID}| ForEach-Object {
